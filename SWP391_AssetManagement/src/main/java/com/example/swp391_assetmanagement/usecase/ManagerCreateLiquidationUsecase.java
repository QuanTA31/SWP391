...
import com.example.swp391_assetmanagement.dto.request.ApprovalLiquidationDTORequest;
...
public class ManagerCreateLiquidationUsecase {
    ...
    @Transactional
    public void execute(ApprovalLiquidationDTORequest request, HttpSession session) {
        ...
        List<AssetExternalRequestDetail> toUpdate = dbDetails.stream()
                .peek(dto -> dto.setExternalStatusId(request.getIsApproved()
                        //check lại logic của in_progress
        ...
        assetRequestService.findAssetRequestByIdForUpdate(request.getAssetRequestId()).ifPresent(
                assetRequest -> {
                    ...
                    assetRequestService.updateLiquidationRequest(assetRequest);
                }
        );
